...
<div class="table-responsive">
    ...
    <form action="{{route('waiting_classrooms.edit_max_waiting_class_rooms')}}" method="POST">
        @method('put')
        @csrf
        <button type="submit" class="btn btn-primary btn-block float-end mb-3">@lang('save')</button>
        <table class="table table-bordered table-hover table-striped table-checkable table-highlight-head mb-4">
            <thead>
                <tr>
                    ...
                    @endforeach
                    <th >{{__("subjects")}}</th>
                    <th>@lang('max_waiting_class_rooms')</th>
                </tr>
            </thead>
            <tbody>
                {!! $module->tableInfo() !!}
