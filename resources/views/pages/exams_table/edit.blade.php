...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{route('exams_table.update',$record->id)}}">
                    @csrf
                    @method('PUT')
                    <div class="form-group mb-5">
                        ...
                        <select class="form-control grades"   name='grades_id' >
                            ...
                            @foreach(AuthLogged()->grades as $grade)
                                <option value="{{$grade->id}}" @selected(collect($record->grades_id)->contains($grade->id) ) > {{$grade->{'name_'.\Config::get('app.locale')} }}  - {{$grade->stage->{'name_'.\Config::get('app.locale')} }}</option>
                            @endforeach
                        </select>
                        @error('grades_id')
                    ...
                    <div class="form-group mb-5">
                        ...
                        <select class="form-control "   name='subjects_id' >
                            ...
                            @foreach(AuthLogged()->subjects as $subject)
                                <option value="{{$subject->id}}" @selected(collect($record->subjects_id)->contains($subject->id) ) > {{$subject->{'name_'.\Config::get('app.locale')} }}  </option>
                            @endforeach
                        </select>
                        @error('subjects_id')
                            <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="form-group mb-5">
                        ...
                        <input type="date" name="date" value="{{$record->date}}" class="form-control form-control-solid">
                        @error('date')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-5">
                        ...
                        <input name="start_time" type="time"  value="{{$record->start_time}}"
                    ...
                    <div class="form-group mb-5">
                        ...
                        <input name="end_time" type="time" id="start_at" value="{{$record->end_time}}"
                               class="form-control form-control-solid">
                        @error("end_time")
                        <span class="text-danger">{{$message}}</span>
                        @enderror
