            ...
            <div class="row ">
                <div class="col-11">
                    <div class="search-input-group-style input-group mb-3">
                        ...
                        <input type="search" autofocus   class="form-control searchAjax" data-ajaxfunction='getRecords' placeholder="@lang('search')" value="{{request()->keyword}}" name='keyword'>
                    </div>
                </div>
                ...
                <div calss="col-md-3 " style="margin:5px">
                    <select class="form-control" name="sortBy" >
                        ...
                        @foreach($module->getSortBy() as $col)
                            <option value="{{$col}}" @selected(request()->sortBy==$col) >{{__(''.$col)}}</option>
                        @endforeach
                        <option value="grades_id" @selected(request()->sortBy=='grades_id') >{{__('gradeName')}}</option>
    ...
    <table class="table table-bordered table-hover table-striped table-checkable table-highlight-head mb-4">
        <thead>
            <tr>
                ...
                @endforeach
                <th class="text-center">{{__("gradeName")}}</th>
                <th>@lang('stage')</th>
                <th class="text-center">@lang('Status')</th>
                <th class="text-center">@lang('class timeTables')</th>
