            ...
            <div class="card-body form-content">
                ...
                <form method="POST" action="{{route('grades.update',$grade->id)}}">
                    ...
                    <div class="form-group mb-5">
                        ...
                        <select class="form-control basic" name="stages_id">
                            ...
                            @foreach(AuthLogged()->stages as $stage)
                                <option value="{{$stage->id}}"  @selected($grade->stages_id == $stage->id) >{{ $stage->name_ar }}</option>
                            @endforeach
                        </select>
                        @error('stages_id')
                    ...
                    <div class="form-group mb-5">
                        ...
                        <input type="text" name="name_ar" value="{{$grade->name_ar}}" id="name_ar" class="form-control form-control-solid">
                        @error('name_ar')
                            <span class="text-danger error-messages">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-5">
                        ...
                        <input type="text" name="name_en" value="{{$grade->name_en}}"  id="name_en" class="form-control form-control-solid">
                        @error('name_en')
                        <span class="text-danger error-messages">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <h4>
                        ...
                        <span id='total'> 
                            {{$grade->grade_subject->sum('individual_portions')+ ($grade->grade_subject->sum('matrimonial_portions')*2) }} 
                        </span >
                    </h4>
                    <br><br>
                    <div class="subjects-records">
                        @foreach($grade->grade_subject as $grade_subject)
                            <div class="row subject-record bg-dark " style="margin:10px;padding:50px">
                                <div class="col-md-1">
                                    <label  class="text-capitalize font-weight-bold text-info subject-record-order">{{$loop->iteration}}</label>
                                </div>
                                <div class="col-md-3">
                                    <label for="" class="{{$loop->iteration > 1?'d-none':''}}  text-capitalize font-weight-bold text-info">@lang('choose subject')</label>
                                    <select class=" form-control subjects_ids selectpicker" data-live-search="true" name='subjects_ids[]'>
                                        @foreach(AuthLogged()->subjects as $subject)
                                            <option value="{{$subject->id}}" @selected($grade_subject->subjects_id==$subject->id) >{{$subject->name_ar}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-3">
                                    <label  class="{{$loop->iteration > 1?'d-none':''}} text-capitalize font-weight-bold text-info">@lang('individual_portions')</label>
                                    <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                                        <input  type="number" value="{{$grade_subject->individual_portions}}" min='0' max='10' name="individual_portions[]" class="form-control individual_portions">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <label  class="{{$loop->iteration > 1?'d-none':''}} text-capitalize font-weight-bold text-info">@lang('matrimonial_portions')</label>
                                    <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                                        <input  type="number"  value="{{$grade_subject->matrimonial_portions}}" min='0' max='5' name="matrimonial_portions[]" class="form-control matrimonial_portions">
                                    </div>
                                </div>
                                @if($loop->first)
                                    <div class="col-md-1">
                        ...
                        @endforeach
                        @if($grade->grade_subject->count()==0)
                            <div class="row subject-record bg-dark " style="margin:10px;padding:50px">
                                <div class="col-md-3">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                                        <input  type="number" value="1" min='0' max='10' name="individual_portions[]" class="form-control individual_portions">
                                    </div>
                                </div>
                                <div class="col-md-3">
