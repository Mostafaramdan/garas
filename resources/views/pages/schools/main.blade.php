            ...
            <div class="row ">
                <div class="col-11">
                    <div class="search-input-group-style input-group mb-3">
                        ...
                        <input type="search" class="form-control searchAjax" data-ajaxfunction='getRecords' placeholder="@lang('search')" value="{{request()->keyword}}" name='keyword'>
                    </div>
                </div>
                ...
                <div calss="col-md-3 " style="margin:5px">
                    <select class="form-control" name="filterSubscription">
                        <option disabled>@lang('filterSubscription')</option>
                        <option value="current" @selected(request()->filterSubscription=='current')>@lang('current')</option>
                        <option value="end" @selected(request()->filterSubscription=='end')>@lang('end')</option>
                    </select>
                </div>
            </div>
        </form>
    ...
    <table class="table table-bordered table-hover table-striped table-checkable table-highlight-head mb-4">
        <thead>
            <tr>
                ...
                @endforeach
                <th class="text-center">@lang('Package Expiry Date')</th>
                @foreach($module->togglableColumns as $togglableColumn)    
                    <th class="text-center">@lang("{$togglableColumn}")</th>
                @endforeach
