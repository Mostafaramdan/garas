            ...
            <table class=" table table-bordered table-hover table-striped table-checkable table-highlight-head mb-4">
                ...
                <tbody  >
                    @foreach($records->unique('teachers_id') as $record)
                        @php $waiting_class_rooms=collect([]); @endphp
                        <tr >
                                ...
                                @foreach($records->pluck('class_room')->unique() as $class_room)
                                    ...
                                    @else
                                        @php
                                            $classRoom = $records
                                                ->where('day',$day)
                                                ->where('waiting_teachers_id',$record['teachers_id'])
                                                ->where('class_number',$class_room->number)
                                                ->first(); 
                                        ...
                                        @if($classRoom) 
                                            <th>
                                                <span class="class_name" style="color:red;" > @lang('waiting') .<br>{{$classRoom['className']}}</span>
                                            </th> 
                                        @else
                                            <th>_</th> 
                                        @endif
...
@push('scripts')
    <script>
        ...
        $("body").on("click",'#change_class_room',function(e){
            $("#change_class_room_modal").modal('toggle');
        });
