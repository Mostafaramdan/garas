                ...
                <div calss="col-md-3 " style="margin:5px">
                    <select class="form-control" name="sortBy">
                        ...
                        @foreach($module->getColumns() as $col)
                            <option value="{{$col}}" @selected(request()->sortBy==$col)
                                >{{__(''.$col)}}</option>
                        @endforeach
                    </select>
                </div>
...
@push('scripts')
    <script>
        $("input[class='switch']").on("input",function(e){
            let checkbox= $(this);
            console.log($("input[class='switch']:checked").length)
            if($("input[class='switch']:checked").length  > 1 ){
                setTimeout(() => {
                    $("input[class='switch']").prop("checked",false);
                    setTimeout(() => {
